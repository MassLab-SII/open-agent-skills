...
class FilesystemStateManager(BaseStateManager):
    ...
    def _create_initial_state(self, task: BaseTask) -> Optional[Dict[str, Any]]:
        ...
        Returns the task directory path and backup path as state info.
        """
        if self.current_task_dir and self.current_task_dir.exists():
            state_info = {"task_directory": str(self.current_task_dir)}
            # Include backup path if it was created
            if self.backup_dir:
                state_info["backup_path"] = str(self.backup_dir)
            return state_info
        return None
    ...
    def _store_initial_state_info(
        ...
        For filesystem, we store the test directory path and backup path.
        """
        if state_info and hasattr(task, "__dict__"):
            if "task_directory" in state_info:
                task.test_directory = state_info["task_directory"]
            if "backup_path" in state_info:
                task.backup_path = state_info["backup_path"]
