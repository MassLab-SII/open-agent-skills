...
class MCPMarkAgent(BaseMCPAgent):
    ...
    MAX_TURNS = 100
    BASE_SYSTEM_PROMPT = (
        "You are a helpful agent that uses skill iteratively to complete the user's task, "
        "and when finished, provides the final answer or simply states \"Task completed\" without further skill calls."
    )
    DEFAULT_TIMEOUT = BaseMCPAgent.DEFAULT_TIMEOUT
    
    # Skill manager instance (shared across all agents)
    ...
    def __init__(
        ...
        if MCPMarkAgent._skill_manager is None:
            # Get the project root directory (two levels up from this file)
            project_root = Path(__file__).parent.parent.parent
            skill_dir = project_root / "skills"
            MCPMarkAgent._skill_manager = SkillManager(skill_dir=str(skill_dir))
    ...
    def _build_system_prompt(self) -> str:
        ...
        Returns:
            Complete system prompt including skill metadata
        """
        base_prompt = self.BASE_SYSTEM_PROMPT
        skill_summary = MCPMarkAgent._skill_manager.get_skill_summary_prompt()
        return base_prompt + skill_summary
    
    def _check_skill_trigger(self, text: str) -> Optional[str]:
        """
        Check if the model's response mentions a skill.
        
        Args:
            text: Text to check for skill mentions
            
        Returns:
            Skill name if triggered, None otherwise
        """
        return MCPMarkAgent._skill_manager.detect_skill_trigger(text)
    
    def _load_skill_context(self, skill_name: str) -> str:
        """
        Load the full SKILL.md content for a triggered skill.
        
        Args:
            skill_name: Name of the skill to load
            
        Returns:
            Full SKILL.md content with usage instructions
        """
        content = MCPMarkAgent._skill_manager.get_skill_full_content(skill_name)
        if content:
            logger.info(f"Loaded full context for skill: {skill_name}")
            return (
                f"\n\n## 🎯 Skill Activated: {skill_name}\n\n"
                f"The '{skill_name}' skill has been activated. Below is the complete documentation.\n\n"
                f"{content}\n\n"
                f"---\n\n"
                f"**Next Steps:**\n"
                f"1. Review the skill documentation above carefully\n"
                f"2. Determine the appropriate command and parameters based on the user's task\n"
                f"3. Output the command using the format specified in the system prompt (code block recommended)\n"
                f"4. After receiving the results, provide a summary to the user\n\n"
                f"**Remember:** You decide the command parameters based on the task requirements and the documentation above.\n"
            )
        return ""
    
    def _extract_and_execute_commands(
        self, 
        text: str, 
        working_dir: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        Extract commands from model output and execute them.
        ...
        Returns:
            List of execution results with format:
            [{"command": str, "success": bool, "stdout": str, "stderr": str}]
        """
        commands = MCPMarkAgent._skill_manager.extract_commands_from_text(text)
        results = []
    ...
    def _format_command_results(self, results: List[Dict[str, Any]]) -> str:
        """
        Format command execution results for inclusion in message history.
        
        Args:
            results: List of command execution results
            
        Returns:
            Formatted string with command outputs
        """
        if not results:
            return ""
    ...
    async def _execute_anthropic_native_tool_loop(
        ...
        for _ in range(max_turns):
            ...
            # Log text output and check for skill triggers
            assistant_text = ""
            ...
            command_results = []
            if self._active_skill and assistant_text:
                working_dir = self.service_config.get("test_directory")
            ...
            # Add skill context or command results if needed
            user_additions = []
            
            # If skill was just triggered, add full skill documentation
            if self._active_skill and turn_count == 1:
                skill_context = self._load_skill_context(self._active_skill)
                if skill_context:
                    user_additions.append({"type": "text", "text": skill_context})
            
            # If commands were executed, add results
            if command_results:
                results_text = self._format_command_results(command_results)
                user_additions.append({"type": "text", "text": results_text})
            
            # If we have additions, add them as a user message
            if user_additions:
                messages.append({"role": "user", "content": user_additions})
                self._update_progress(messages, total_tokens, turn_count)
        ...
        return {
            "success": True,
            "output": sdk_format_messages,
            "raw_messages": messages,  # Add raw messages
            "token_usage": total_tokens,
            "turn_count": turn_count,
            "error": None,
            "litellm_run_model_name": self.litellm_run_model_name,
    ...
    async def _execute_litellm_tool_loop(
        ...
        try:
            while turn_count < max_turns:
                ...
                # For custom providers (like gzy/), tell litellm to treat as OpenAI-compatible
                if self.litellm_input_model_name.startswith("gzy/"):
                    completion_kwargs["custom_llm_provider"] = "openai"
                
                try:
                    # Call LiteLLM with timeout for individual call
                    # debug_messages = [{"role":"system", "content": "You are a helpful assistant."},
                    #     {"role": "user", "content": "hi"}]
                    # debug_completion_kwargs = {
                    #     "model": "gpt-5.1",
                    #     "api_key": self.api_key,
                    #     "base_url": self.base_url,
                    #     "messages": debug_messages,
                    # }
                    # debug_response = await asyncio.wait_for(
                    #     litellm.acompletion(**debug_completion_kwargs),
                ...
                # If skill was just triggered, add full skill documentation
                if self._active_skill and not skill_context_added:
                    skill_context = self._load_skill_context(self._active_skill)
