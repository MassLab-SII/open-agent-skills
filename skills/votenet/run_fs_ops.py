...
This script runs filesystem operation code snippets without requiring full Python boilerplate.
Unlike browser operations, filesystem operations don't need a persistent server.
...
Example operations.txt:
    await fs.read_text_file("/path/to/file.txt")
    await fs.list_directory("/path/to/directory")
    await fs.write_file("/path/to/new_file.txt", "content")
"""
